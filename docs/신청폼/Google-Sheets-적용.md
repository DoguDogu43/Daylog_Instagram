...
적용 전 구현 파일이 다음 v2 계약과 일치하는지 확인합니다.

- `schemaVersion`: `daylog-life-session-v2`
- `comfortableTime`, `difficultTime`: 앞뒤 공백 제거 후 각각 1~100자 문자열
- `togetherStyle`: 요구하거나 저장하지 않음
- 퍼널: `together_style_selected`, `life_note_viewed`를 사용하지 않음
...
- 4단계 다음 `application_started`가 집계되고 제거된 두 퍼널 단계가 현황 탭에 생기지 않는지 확인합니다.
- 신규 신청응답 탭이 v2 24열 구조로 생성되고 에너지 순간 문자열이 그대로 저장되는지 확인합니다.
- 기존 v1 탭은 적용 전에 백업합니다. 적용 후 `함께하는 방식` 헤더가 `v1 보관 필드 (수집 중단)`으로 바뀌되 기존 셀 값이 유지되는지 확인합니다.
- 기존 25열 탭의 새 v2 행은 헤더명 기준으로 정렬되고 보관 필드가 빈 값인지 확인합니다. Apps Script는 기존 데이터 열을 삭제하거나 덮어쓰지 않습니다.
